...
<table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
	...
	<?php foreach ($users as $user) :
		...
		if (in_array('varejista', $user_roles) || in_array('oficina', $user_roles)) :
			$user_status = strtoupper(get_user_meta($user->ID, 'user_status', true));
				...
				<tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
					...
					<td class="px-6 py-4">
						...
						<button data-modal-target="default-modal-<?php echo $user->ID; ?>"
								...
								class="block text-white bg-green-600 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button">
							Editar
						</button>
						<!-- Main modal -->
								...
								<div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
									...
									<div class="p-4 md:p-5 space-y-4">
										...
										<form id="editUserForm-<?php echo $user->ID; ?>"
											...
											<div class="col-span-1">
												<label for="editUserStatus<?php echo $user->ID; ?>"
													   class="font-medium text-gray-700 dark:text-gray-300">Status:</label>
												<select name="user_status"
														id="editUserStatus<?php echo $user->ID; ?>"
														class="w-full border border-gray-300 rounded px-3 py-2">
													<option value="pendente"
														<?php echo ($user_status == 'PENDENTE') ? 'selected' : ''; ?>>
														Pendente
													</option>
													<option value="ativo"
														<?php echo ($user_status == 'ATIVO') ? 'selected' : ''; ?>>
														Ativo
													</option>
													<option value="reprovado"
														<?php echo ($user_status == 'REPROVADO') ? 'selected' : ''; ?>>
														Reprovado
													</option>
													<option value="inativo"
														<?php echo ($user_status == 'INATIVO') ? 'selected' : ''; ?>>
														Inativo
													</option>
												</select>
											</div>
											...
											<!-- Modal footer -->
											<div
												class="col-span-3 flex items-center justify-end p-4 md:p-5 border-t border-gray-200 rounded-b dark:border-gray-600">
